...
export default function ArchiveDocsListPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // 1) boards에서 문서자료실(board.code = 'archive_docs') 찾기
      const { data: board, error: boardError } = await supabase
        .from('boards')
        .select('id')
        .eq('code', 'archive_docs')
        .single()

      if (boardError || !board) {
        setError(
          '문서자료실 게시판(boards.code = archive_docs)을 찾을 수 없습니다.'
        )
        setLoading(false)
        return
      }
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12 max-w-[1400px] mx-auto">
              {posts.map((post) => {
                ...
                const category = post.label || '문서자료' // 기본값
                ...
                return (
                  <NoticeCard
                    ...
                    href={`/archive/docs/detail?id=${post.id}`}
                  />
                )
              })}
            </div>
